...
static void reset_cb(lv_event_t * e)
{
    ...
    while((MADHT1505BA1_run_position_acquisition(&slave[0])/CONVERSION_ANGULAR != 0) ||
...
static void *motor_sync_th(void *arg) {
	...
	while(1) {
        ...
        while((MADHT1505BA1_run_position_acquisition(&slave[0])/CONVERSION_ANGULAR != 360) ||
...
static void meter_cb(lv_event_t *e)
{
    ...
    lv_label_set_text_fmt(position_ctrl_target->target_val, "%3.2f",
        final_angle / 1.0);
#if ENABLE_MOTOR_CONTROL
	MADHT1505BA1_motor_set_position_run(CONVERSION_ANGULAR*final_angle, position_ctrl_target->slave);
#endif
}
...
static void ui_init(void)
{
    ...
    lv_obj_set_style_text_font(label_jitter_val, ttf_main_s.font,
        LV_PART_MAIN);
    lv_obj_align_to(label_jitter_val, label_jitter, LV_ALIGN_OUT_RIGHT_BOTTOM, 50, 0);
...
#if ENABLE_MOTOR_CONTROL
    cur_pos = MADHT1505BA1_run_position_acquisition(&slave[0]); // CONVERSION_ANGULAR
