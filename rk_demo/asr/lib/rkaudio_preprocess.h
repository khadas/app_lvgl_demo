...
typedef struct RKCNGParam_
{
    /*CNG Parameter*/
    float
    fGain;                     /* INT16 Q0 施加舒适噪声幅度比例 */
    float
    fMpy;                        /* INT16 Q0 白噪随机数生成幅度 */
    float              fSmoothAlpha;              /* 舒适噪声平滑系数 */
    float
    fSpeechGain;               /* 根据语音能量额外施加舒适噪声比例增益 */
} RKCNGParam;
...
inline static void *rkaudio_dereverb_param_init()
{
    ...
    param->T60 =
        0.3;//1.5;     /* 混响时间估计值（单位：s），越大，去混响能力越强，但是越容易过消除 */
    param->coCoeff =
        1;         /* 互相干性调整系数，防止过消除，越大能力越强，建议取值：0.5到2之间 */
    return (void *)param;
}
...
inline static void *rkaudio_aes_param_init()
{
    RKAudioAESParameter *param = (RKAudioAESParameter *)malloc(sizeof(
                                     RKAudioAESParameter));
    param->Beta_Up = 0.005f; /* 上升速度 -3588.0f to compatible old json*/
    param->Beta_Down = 0.001f; /* 下降速度 */
    param->Beta_Up_Low = 0.003f; /* 低频上升速度 */
...
inline static void *rkaudio_agc_param_init()
{
    ...
    param->max_gain = 10;           /* 最大增益，同时也是线性段增益，单位：dB */
    param->max_peak =
        -1.0;         /* 经AGC处理后，输出语音的最大能量，范围：单位：dB */
    param->fRk0 = 2;                /* 扩张段斜率 */
    param->fRth2 =
        -15;             /* 压缩段起始能量dB阈值，同时也是线性段结束阈值，增益逐渐降低，注意 fRth2 + max_gain < max_peak */
    param->fRth1 =
        -50;             /* 扩张段结束能量dB阈值，同时也是线性段开始阈值，能量高于改区域以max_gain增益 */
    param->fRth0 = -55;             /* 噪声门阈值 */
...
inline static void *rkaudio_cng_param_init()
{
    ...
    /* cng paremeters */
    param->fSmoothAlpha =
        0.99f;                                                    /* INT16 Q15 施加舒适噪声平滑度 */
    param->fSpeechGain =
        0;                                                     /* INT16 Q15 施加舒适噪声语音纹理模拟程度 */
    param->fGain =
        10.0;                                           /* INT16 Q0 施加舒适噪声幅度比例 */
    param->fMpy =
        10;                                            /* INT16 Q0 白噪随机数生成幅度 */
    return (void *)param;
}
